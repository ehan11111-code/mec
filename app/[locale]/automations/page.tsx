...
import { Clock, Zap, Workflow, ShieldCheck, ChevronRight } from 'lucide-react'
        ...
        <section key={kind} className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
            {automations.filter(a => a.kind === kind).map(a => {
              ...
              return (
                <Panel key={a.id}>
                  <Link href={`/automations/${a.id}`} className="block group/card -m-1 p-1 rounded-soft">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-sm font-semibold text-text leading-snug group-hover/card:text-accent transition-colors">{a.name[locale]}</h3>
                    ...
                    <span className="mt-3 inline-flex items-center gap-1 text-[11px] font-medium text-accent group-hover/card:underline">{t('openDocs')}<ChevronRight className="h-3.5 w-3.5 flip-rtl" strokeWidth={2} /></span>
                  </Link>
