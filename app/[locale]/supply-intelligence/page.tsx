...
import { Radar, ExternalLink, ShieldAlert, Loader2, TrendingUp, TrendingDown, Minus, Truck, Gauge, Globe } from 'lucide-react'
              ...
              <Panel key={d.supplier}>
                ...
                {(po?.drivers || []).length > 0 && (
                  <ul className="mt-3 space-y-1.5 border-t border-border/60 pt-2.5">
                    {po!.drivers.map((dr, i) => (
                      <li key={i} className="text-[11px] text-text-soft leading-snug">↗ {dr.summary}
                        {dr.citation?.url && <a href={dr.citation.url} target="_blank" rel="noopener noreferrer" className="ms-1 inline-flex items-center gap-0.5 text-accent/90 hover:text-accent"><ExternalLink className="h-2.5 w-2.5" strokeWidth={1.8} />{dr.citation.source || t('source')}{dr.citation.date ? ` · ${dr.citation.date}` : ''}</a>}
                      </li>
                    ))}
                  </ul>
                )}
                  ...
                  <div className="mt-3 space-y-2 border-t border-border/60 pt-2.5">
                    {(d.risks || []).map((r, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className={clsx('shrink-0 mt-0.5 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium capitalize', SEV[r.severity] || SEV.low)}>{t(`sev_${r.severity}`)}</span>
                        <p className="text-[11px] text-text-soft leading-snug">{r.summary}
                          {r.citation?.url && <a href={r.citation.url} target="_blank" rel="noopener noreferrer" className="ms-1 inline-flex items-center gap-0.5 text-accent/90 hover:text-accent"><ExternalLink className="h-2.5 w-2.5" strokeWidth={1.8} />{r.citation.source || t('source')}{r.citation.date ? ` · ${r.citation.date}` : ''}</a>}
                        </p>
                      </div>
                    ))}
                  </div>
