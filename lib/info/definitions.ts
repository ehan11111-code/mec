...
export const DEFS: Record<string, Def> = {
  ...
  onHand: { def: { en: 'Cartons in the warehouse now. Use the icons to switch the source: JARVIS reconciled (ledger net of recorded in/out, excluding unbalanced SKUs), Tarek’s physical المخزون count, or available-after-open-orders.', ar: 'الكراتين في المستودع الآن. استخدم الأيقونات لتبديل المصدر: تسوية جارفيس (صافي السجل وارد/صادر، باستثناء الأصناف غير المتوازنة)، أو جرد طارق الفعلي (المخزون)، أو المتاح بعد الطلبات المفتوحة.' }, source: { en: 'Warehouse ledger (المخزون 2026) + the المخزون physical count + live open orders from whatsapp_intake.', ar: 'سجل المستودع (المخزون 2026) + الجرد الفعلي (المخزون) + الطلبات المفتوحة المباشرة من whatsapp_intake.' } },
