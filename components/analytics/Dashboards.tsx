...
function MonthLine({ filter, locale, title }: P & { title: string }) {
  ...
  return <LineChartPanel data={data} series={[{ key: 'revenue', label: t('kRevenue'), accent: true }]} title={title} locale={locale} height={260} />
}
...
function CategoryDonut({ filter, locale, title }: P & { title: string }) {
  ...
  return (
    <Panel title={title}>
      <DonutStat centerValue={fmtSAR(cats.reduce((s, c) => s + c.v, 0))} centerLabel=""
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard label={t('kRevenue')} value={fmtSAR(s.revenue)} accent index={0} />
        <StatCard label={t('kSpend')} value={fmtSAR(proc.spend)} index={1} />
        <StatCard label={t('kGrossProfit')} value={fmtSAR(grossProfit)} accent index={2} />
        <StatCard label={t('kReceivables')} value={fmtSAR(s.outstanding)} index={3} />
      </section>
      <section className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard label={t('kInvoices')} value={f0(s.invoices)} index={0} />
        <StatCard label={t('kClients')} value={f0(s.clients)} index={1} />
        <StatCard label={t('kVat')} value={fmtSAR(s.vat)} index={2} />
        <StatCard label={t('kAvgInvoice')} value={fmtSAR(s.avgInvoice)} index={3} />
      </section>
...
export function SalesDashboard({ filter, locale }: P) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
        <MonthLine filter={filter} locale={locale} title={t('revByMonth')} />
        <BarChartPanel locale={locale} title={t('revBySalesperson')}
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-3">
        <StatCard label={t('kSpend')} value={fmtSAR(proc.spend)} accent index={0} />
        <StatCard label={t('kSuppliers')} value={f0(proc.suppliers)} index={1} />
        <StatCard label={t('kPurchaseLines')} value={f0(proc.lines)} index={2} />
      </section>
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
        <BarChartPanel locale={locale} title={t('spendBySupplier')} height={300}
      ...
      <p className="text-xs text-muted">{t('procNote')}</p>
    </div>
  )
}
...
export function MarginDashboard({ locale }: P) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <BarChartPanel locale={locale} title={t('marginByCategory')} subtitle={t('marginSub')}
        ...
        <div className="overflow-x-auto scrollbar-soft">
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-muted border-b border-border">
              ...
              <th className="text-end font-medium px-4 py-3">{t('avgSell')}</th>
              <th className="text-end font-medium px-4 py-3">{t('avgCost')}</th>
              <th className="text-end font-medium px-4 py-3">{t('marginPct')}</th>
              <th className="text-end font-medium px-5 md:px-6 py-3">{t('kRevenue')}</th>
            </tr></thead>
            <tbody className="divide-y divide-border">
              {rows.map((r, i) => (
      ...
      <p className="text-xs text-muted">{t('marginNote')}</p>
    </div>
  )
}
...
export function CollectionsDashboard({ filter, locale }: P) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="grid gap-3 md:gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard label={t('collected')} value={fmtSAR(c.collected)} accent index={0} />
        <StatCard label={t('outstanding')} value={fmtSAR(c.outstanding)} index={1} />
        <StatCard label={t('cash')} value={fmtSAR(c.cash)} index={2} />
        <StatCard label={t('bank')} value={fmtSAR(c.bank)} index={3} />
      </section>
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-6">
        <Panel title={t('collectedVsOutstanding')}>
          <DonutStat centerValue={fmtSAR(c.total)} centerLabel={t('kRevenue')}
            segments={[{ label: t('collected'), value: Math.max(0, c.collected), accent: true }, { label: t('outstanding'), value: Math.max(0, c.outstanding) }]} />
        </Panel>
        <LineChartPanel locale={locale} title={t('revByMonth')}
...
export function ProductsDashboard({ filter, locale }: P) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <BarChartPanel locale={locale} title={t('topByRevenue')} height={300}
      ...
      <BarChartPanel locale={locale} title={t('topByVolume')} height={260}
